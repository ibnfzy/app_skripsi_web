        ...
        <aside class="w-72 bg-[var(--biru-tua)] text-white flex flex-col shadow-xl">
            ...
            <nav class="flex-1 overflow-y-auto px-4 py-6 space-y-2">
                <?php foreach (($menu ?? []) as $index => $item): ?>
                    <a href="#" class="block px-4 py-3 rounded-xl <?php echo $index === 0 ? 'bg-white text-[var(--biru-tua)] font-semibold shadow' : 'text-white/90 hover:bg-white/10'; ?> transition">
            ...
            <div class="px-6 py-4 border-t border-white/10">
                <button class="w-full py-3 rounded-lg bg-white/10 hover:bg-white/20 text-white font-semibold transition">Logout</button>
        ...
        <main class="flex-1">
            <header class="bg-white shadow-sm">
                <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="h-10 w-10 rounded-full bg-[var(--biru-medium)] text-white flex items-center justify-center font-bold">ID</div>
                    </div>
                </div>
            </header>
            ...
            <section class="max-w-6xl mx-auto px-6 py-10 space-y-8">
                <div class="grid md:grid-cols-[2fr,1fr] gap-6">
                    <div class="p-6 rounded-2xl bg-white shadow-sm border border-slate-100">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p class="mt-2 text-slate-600 leading-relaxed"><?= esc($descriptions[$role] ?? 'Akses modul sesuai peran Anda.'); ?></p>
                            </div>
                    ...
                    <div class="p-6 rounded-2xl bg-gradient-to-br from-[var(--biru-tua)] to-[var(--biru-medium)] text-white shadow-lg">
                        <p class="text-sm text-white/80">Informasi Singkat</p>
                        <h3 class="text-2xl font-semibold mt-2">Alur Komponen</h3>
                        <p class="text-white/80 mt-2">Panel memuat modul dinamis berdasarkan role. Gunakan menu samping untuk navigasi.</p>
                        <div class="mt-4 space-y-2 text-sm">
                            <div class="flex items-center gap-2"><span class="h-2 w-2 rounded-full bg-white"></span>Sidebar menampilkan menu sesuai role.</div>
                            <div class="flex items-center gap-2"><span class="h-2 w-2 rounded-full bg-white"></span>Topbar menampilkan identitas pengguna.</div>
                            <div class="flex items-center gap-2"><span class="h-2 w-2 rounded-full bg-white"></span>Konten dinamis berdasarkan modul aktif.</div>
                        </div>
                        <button class="mt-5 w-full py-3 rounded-lg bg-white text-[var(--biru-tua)] font-semibold hover:translate-y-[-2px] transition">Lihat Modul Aktif</button>
                    </div>
                </div>

                <div class="grid md:grid-cols-3 gap-4">
                    <?php foreach ($modules as $title => $desc): ?>
                        <div class="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm hover:-translate-y-1 hover:shadow-lg transition">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div class="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm">
                        ...
                        <p class="text-sm text-slate-600">Load view dan komponen otomatis sesuai role: mahasiswa, dosen, kaprodi, atau sekjur. Setiap modul siap dihubungkan dengan data backend.</p>
                    ...
                    <div class="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm">
                        ...
                        <p class="text-sm text-slate-600">Sesi aman dengan alur destroy_session dan redirect_login. Integrasikan dengan middleware autentikasi.</p>
                        <div class="mt-4 p-4 rounded-xl bg-[var(--abu-muda)] border border-dashed border-[var(--biru-medium)] text-sm text-[var(--biru-tua)]">Klik tombol logout pada sidebar untuk keluar dari sistem.</div>
                    </div>
                </div>
            </section>
        </main>
